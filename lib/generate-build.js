...
//  options (Object):
//    dest (String, Boolean): Destination path for the custom Modernizr build,
//      or false to output nothing. Defaults to './modernizr.js'.
...
module.exports = function build(config, options) {
  ...
  requirejs.optimize(rjsConfig, function () {
    ...
    if (options.dest) {
      ...
      try {
        fs.writeFile(options.dest, output);
        if (options.verbose) {
          console.log('Custom Modernizr build written to ' + options.dest + '!');
        }
      }
      catch (err) {
        console.log(err);
