...
module.exports = function build(config, options) {
  ...
  requirejs.optimize(rjsConfig, function () {
    ...
    out = out.replace('define("modernizr-init", function(){});', '', 'g');

    // Hack the prefix into place. Anything is way too big for something so
    // small.
    if (config.classPrefix) {
      out = out.replace('classPrefix: \'\',', 'classPrefix: \'' + config.classPrefix.replace(/"/g, '\\"') + '\',');
    }

    // Uglify
    if (options.min) {
      out = uglifyjs.minify(out, {
        fromString: true,
        mangle: {
          except: 'Modernizr'
        }
    ...
    if (options.dest) {
      fs.writeFileSync(options.dest, out);
    }
